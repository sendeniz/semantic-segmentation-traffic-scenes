...
def main():
    ...
    optimizer = optim.Adam(fcnn_model.parameters(), lr = lr, weight_decay = weight_decay)
    ...
    for epoch in range(epochs):
        loss_value, iou_avg_val, pixel_avg_val  = train(train_loader, fcnn_model, optimizer, loss_f)
        loss_lst.append(loss_value)
        iou_avg.append(iou_avg_val)
        pixel_avg_acc.append(pixel_avg_val)

        loss_value_test, iou_avg_val_test, pixel_avg_val_test  = test(test_loader, fcnn_model, loss_f)
        ...
        if epoch == epochs - 1: 
            with open('results/fcnn8_loss.txt','w') as values:
                values.write(str(loss_lst))
            with open('results/fcnn8_iou.txt','w') as values:
                values.write(str(iou_avg))
            with open('results/fcnn8_pixel_acc.txt','w') as values:
                values.write(str(pixel_avg_acc))
            with open('results/fcnn8_loss_test.txt','w') as values:
                values.write(str(loss_lst_test))
            with open('results/fcnn8_iou_test.txt','w') as values:
                values.write(str(iou_avg_test))
            with open('results/fcnn8_pixel_acc_test.txt','w') as values:
